...
export function Header({ sections, isGalleryPage = false }: HeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-50 w-full">
      <div className="max-w-7xl w-full mx-auto px-4 sm:px-6 py-4">
        <div className="relative">
          <nav className="w-full border border-white/10 shadow-lg shadow-emerald-500/5 rounded-full px-6 sm:px-4 py-3 flex items-center justify-between bg-white/5 backdrop-blur-[4px] supports-[backdrop-filter]:bg-white/5 supports-not-[backdrop-filter]:bg-black/30">
            {/* Logo */}
            <Link 
            ...
            <div className="md:hidden">
              ...
              >
                <div className="relative w-6 h-6 flex items-center justify-center">
                  <span className={cn("block h-0.5 w-6 bg-current transition-all duration-300 ease-in-out", open ? "rotate-45" : "-translate-y-1.5")} />
                  <span className={cn("absolute h-0.5 w-6 bg-current transition-all duration-300 ease-in-out", open ? "opacity-0" : "opacity-100")} />
                  <span className={cn("block h-0.5 w-6 bg-current transition-all duration-300 ease-in-out absolute", open ? "-rotate-45" : "translate-y-1.5")} />
                </div>
          ...
          <div 
            className={cn(
              "md:hidden fixed left-1/2 -translate-x-1/2 top-24 z-50 backdrop-blur-lg border border-white/10 shadow-2xl transition-all duration-200 ease-out overflow-hidden w-[calc(100%-2rem)] rounded-2xl",
